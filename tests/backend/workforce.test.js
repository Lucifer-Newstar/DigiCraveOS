...
describe("Phase 2 shifts and payroll", () => {
  test("Admin can start and close a staff shift", async () => {
    ...
    const staff = await request(app).post("/api/user/register").send({ name: `Shift Staff ${Date.now()}`, email: `shift_${Date.now()}@test.com`, phone: uniquePhone(), password: "pass1234", role: "Cashier" });
  ...
  test("Admin cannot start a second open shift for the same staff member", async () => {
    const { cookie } = await loginAsAdmin();
    const staff = await request(app).post("/api/user/register").send({ name: `Open Staff ${Date.now()}`, email: `open_${Date.now()}@test.com`, phone: uniquePhone(), password: "pass1234", role: "Waiter" });
