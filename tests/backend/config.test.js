describe("Phase 4 configuration validation", () => {
  ...
  test("rejects invalid slow-query thresholds", () => {
    const { validateConfig } = require("../../Restaurant_POS_System/pos-backend/config/config");
    expect(validateConfig({ port: 3000, databaseURI: "mongodb://localhost/test", nodeEnv: "test", slowQueryMs: -1 })).toContain("SLOW_QUERY_MS must be a non-negative number.");
  });
