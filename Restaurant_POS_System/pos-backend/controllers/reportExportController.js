...
const exportOrdersCsv = async (req, res, next) => {
  try {
    ...
    res.type("text/csv").set("ETag", etag).set("Content-Length", String(Buffer.byteLength(csv))).set("Cache-Control", "private, no-cache").set("Content-Disposition", `attachment; filename="orders-${from.toISOString().slice(0, 10)}-to-${to.toISOString().slice(0, 10)}.csv"`).send(csv);
