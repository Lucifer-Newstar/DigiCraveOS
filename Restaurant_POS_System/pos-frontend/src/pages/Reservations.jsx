...
const Reservations = () => {
  ...
  return <section className="pos-page"><div className="pos-page-header"><div><h1 className="pos-title">Reservations</h1><p className="pos-subtitle">Bookings and simple waitlist management</p></div></div>
    ...
    <div className="flex items-center justify-between gap-3 mt-3 text-sm"><span className="text-slate-500">Page {data?.pagination?.page || page} of {data?.pagination?.pages || 1} · {data?.pagination?.total || reservations.length} reservations</span><div className="flex gap-2"><button className="pos-btn-ghost" disabled={page <= 1} onClick={() => setPage((current) => current - 1)}>Previous</button><button className="pos-btn-ghost" disabled={page >= (data?.pagination?.pages || 1)} onClick={() => setPage((current) => current + 1)}>Next</button></div></div>
  </section>;
};
